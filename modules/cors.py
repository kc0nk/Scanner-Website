...
class CORSModule(ExploitModule):
    ...
    async def run(self, ctx):
        ...
        for payload in probes:
            ...
            try:
                ...
                if allow_origin in {header.get("Origin", ""), "*"}:
                    ...
                    ctx.artifacts.set("cors.hit", True)
        ...
        return ExploitResult(self.name, "signal" if ctx.artifacts.get("cors.hit") else "no-signal", "CORS probes completed (all payloads tested)", evidence="\n".join(evidence[:20]))
