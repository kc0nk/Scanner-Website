        ...
        for endpoint in endpoints[:40]:
            ...
            for idx, (name, _) in enumerate(params[:8]):
                for payload in probes:
                    payload_counter += 1
                    ctx.logger(f"[payload {payload_counter}/{len(probes)}] SSTI: {payload}")
                    mutated = params.copy()
                    ...
                    try:
                        ...
                        if "49" in resp.text and payload != "${7*7}":
                            msg = f"Potential expression evaluation: {name} using {payload}"
                            evidence.append(msg)
                            ctx.artifacts.set("execution.ssti", {"endpoint": probe, "parameter": name, "payload": payload})
                            ctx.add_finding(probe, payload, self.name, f"HTTP {resp.status_code}; expression result observed", confidence="high")
                    except Exception as exc:
                        evidence.append(str(exc))
        return ExploitResult(self.name, "signal" if evidence else "no-signal", "Template probes completed", evidence="\n".join(evidence[:20]))
