...
class SQLiModule(ExploitModule):
    ...
    async def run(self, ctx):
        ...
        try:
            ...
            candidates.extend(ctx.discover_links(final_url, text))
        except Exception:
            pass
        ...
        for candidate in candidates:
            ...
            if not params or candidate in seen:
                continue
            seen.add(candidate)
            for idx, (name, value) in enumerate(params[:5]):
                for payload in payloads:
                    ...
                    try:
                        normal = await ctx.http.request("GET", candidate)
                        test = await ctx.http.request("GET", probe)
